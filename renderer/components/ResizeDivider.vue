...
interface ResizeDividerProps {
  ...
  valIsNegative?: boolean;
  size: number;
  maxSize: number;
  minSize: number;
}
interface ResizeDividerEmits {
  (e: 'update:size', size: number): void;
}

defineOptions({ name: 'ResizeDivider' });

const props = withDefaults(defineProps<ResizeDividerProps>(), {
  valIsNegative: false
});
const emit = defineEmits<ResizeDividerEmits>();
...
function handleDrag(e: MouseEvent) {
  ...
  const diffX = props.valIsNegative ? startPoint.x - e.clientX : e.clientX - startPoint.x;
  const diffY = props.valIsNegative ? startPoint.y - e.clientY : e.clientY - startPoint.y;
